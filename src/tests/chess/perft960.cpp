...
using pair_type = std::pair<std::string, std::array<std::uint64_t, 6>>;

TEST_CASE("Perft960 - suite") {
    const std::array<pair_type, 960> tests = {{
    ...
    for (std::size_t i = 0; i < 6; ++i) {
        ...
        for (const auto &[fen, nodes] : tests) {
            if (nodes.size() > i) {
                INFO("FEN: ", fen);
                INFO("Depth: ", i + 1);
                INFO("Position ", test_num, "/", tests.size());
