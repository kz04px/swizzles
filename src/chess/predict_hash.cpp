...
[[nodiscard]] std::uint64_t Position::predict_hash(const Move &move) const noexcept {
    ...
    switch (move.type()) {
        ...
        case MoveType::ksc:
            ...
            new_hash ^= zobrist::piece_key(Piece::Rook, us, ksc_rook_fr[static_cast<int>(us)]);
            new_hash ^= zobrist::piece_key(Piece::Rook, us, ksc_rook_to[static_cast<int>(us)]);
            break;
        case MoveType::qsc:
            new_hash ^= zobrist::piece_key(Piece::King, us, move.from());
            new_hash ^= zobrist::piece_key(Piece::King, us, move.to());
            new_hash ^= zobrist::piece_key(Piece::Rook, us, qsc_rook_fr[static_cast<int>(us)]);
            new_hash ^= zobrist::piece_key(Piece::Rook, us, qsc_rook_to[static_cast<int>(us)]);
            break;
        case MoveType::promo:
