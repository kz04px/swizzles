...
void Position::legal_noncaptures(std::vector<Move> &moves) const noexcept {
    ...
    if (ch.count() == 1) {
        ...
        UCI_ASSERT(piece_on(sq) != Piece::None);
        UCI_ASSERT(piece_on(sq) != Piece::King);
    ...
    UCI_ASSERT(pinned_pieces == pinned());
    UCI_ASSERT(rook_pinned == (rook_pinned & (kfile | krank)));
    ...
    for (std::size_t i = start_size; i < moves.size(); ++i) {
        UCI_ASSERT(!moves[i].is_capturing());
        UCI_ASSERT(moves[i].captured() == Piece::None);
    }
#endif
}
